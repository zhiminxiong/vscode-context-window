...
const relationsTool: McpToolDefinition = {
    ...
    description: [
        ...
        '"mode"; you cannot ask for one or the other. A method that Call Hierarchy can prepare stays',
        'in call mode even if document symbols only expose the enclosing type.',
        'This is real call-graph data, so it follows overrides (same-named class/interface slots',
        'on the type’s ancestor chain; call sites are classified by enclosing type, not a this. token;',
        'nearest on-chain caller, external receivers kept, sibling-hierarchy omitted; this.xxx()',
        'callees resolve to the most-derived override on that same chain), skips arrow / <function>',
        'callers in favor of the enclosing named method, follows super calls, and marks recursion,',

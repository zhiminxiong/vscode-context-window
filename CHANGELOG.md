...
- Relation: an override lists callers of every same-named class/interface slot on its ancestor chain. Ancestor-chain `this.xxx()` keeps only the nearest (for example `MoveableActionBase.Prepare` for `OnPrepare`). Sibling-hierarchy `this.xxx()` is not added. Calls through a receiver such as `action.InterruptByController()` are kept. A `this.xxx()` outgoing call whose name has `override` on that chain shows the implementation closest to the current class (the current class if it implements it), not the original base declaration. `super` / `base` / `::` still go to the parent implementation. Incoming callers that are arrows / `<function>` / callbacks are shown as the enclosing named method (for example `CreateTimelineTask` instead of the arrow that calls `this.GetTimelineAsset()`), using a real Call Hierarchy item so that method’s own callers can still expand.

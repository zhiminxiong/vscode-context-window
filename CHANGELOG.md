...
- Main editor line blame: the annotation is dropped as soon as the cursor leaves the line or the document is edited, rather than staying up until the next answer arrives. Decorations move with the text they are anchored to, so inserting a line above used to drag the previous line's annotation along with it, and editing a line left git's answer describing text that was no longer there. Moving within one line still leaves it alone, so holding an arrow key does not make it blink.
...
- Git line blame now follows the cursor in the **main VS Code editor**, not just inside the Context Window (`contextView.editor.lineBlame`, on by default), so GitLens is no longer needed for that annotation — both draw at the end of the line, so turn one of them off. Author, relative time, and commit subject appear in gray after the code; the hover carries the full commit message and an **Open Changes** link, which opens the same two-commit diff the Context Window hover does. The annotation is requested only once the cursor stops moving, and the answers come from the same cache the Context Window uses, so holding an arrow key does not spawn a `git blame` per line and the two views cannot disagree. While a line has unsaved edits it reads "Uncommitted changes" instead of a commit: `git blame` numbers lines by the file on disk, so after an insertion or deletion its answer no longer lines up with the screen, and naming the wrong author is worse than naming none. Untitled buffers, virtual documents such as the left side of a diff, and multi-line selections are left alone. The hover reproduces the Context Window's card rather than inventing a second layout, down to the three bands separated by rules: the author with their avatar, a relative time and the exact date, the commit message and the short SHA on its own chip; then this line's additions and deletions; then the same `Changes <parent> ↔ <sha>` / `Changes added in <sha>` footer, with the wording taken from the same rules the webview uses. The rules are literal `---`, which the hover stretches to the full card width; handing the editor one markdown string per band does *not* produce them, because a decoration's strings all land in the same hover part and the rule is drawn between parts. Each SHA sits on a chip with the git-commit node inside it, green for the commit and muted for its parent, and clicking one copies the full SHA — the chips are links, which is the only way a hover offers a pointer cursor, a tooltip, and a click. Each band also carries a copy button for its own text, in the same two formats the Context Window copies. Colors are hard-coded hex, the only kind a hover accepts, so there is one set for dark themes and one for light, reselected when the theme changes; the chip is a flat block padded with spaces, because `padding` and `border-radius` are not on the whitelist, and it gives up the monospace font since `.monaco-hover code` would paint its own background over the chip's. What has no way through at all: font size, line height, and the widget's padding; pushing the date to the card's right edge, which needs flex; a pressed color and buttons that appear only on hover, which need `:active` and `:hover` in a stylesheet we do not have; and the character-level highlight inside a changed line, so the diff is colored per line. Authors with no Gravatar get its generic silhouette, since a hover image cannot fall back to the initial badge. The gray of the annotation itself is a new theme color, `contextView.lineBlameForeground`, defaulting to the translucent `#99999959` GitLens uses — the first version borrowed CodeLens's foreground, which is meant to be read and so stood out next to the code.
- MCP: the extension serves its code intelligence to AI assistants over the Model Context Protocol (`contextView.mcp.enabled`, on by default). Two tools: `code_relations` returns the callers and callees of a function as a tree with the exact call sites, or every reference to a variable or type grouped by the function containing it — the same language-server data the Relation panel draws — and `enclosing_symbol` returns the smallest function, method, class, or namespace containing a line, with its exact range and source. Because the answers come from this window's language servers, the server runs inside the extension; it listens on loopback only, on a port chosen at startup, and requires a bearer token regenerated on every start. VS Code 1.101 and later need no configuration, as the editor asks the extension for the address each time it connects; the API is detected at runtime, so older versions are unaffected. Every other client is configured from a file, which cannot hold an address that changes every restart, so **Configure MCP Server Entry** adds an entry pointing at a bridge script that looks the endpoint up when it is launched, and reconnects by itself after the window restarts on a new port. The entry is an ordinary stdio server run with `node`, not something tied to the editor that wrote it, and the command offers Cursor's and CodeBuddy's files both in the project and in the home directory, anything listed in `contextView.mcp.configFiles`, a path given once, or copying the entry for any other client. VS Code's own file is deliberately not offered, since it already has the server through the API and a file would register the same tools twice. The chosen file is edited in place rather than parsed and rewritten, so servers already configured there keep their settings and credentials along with their comments, key order, and formatting; comments and trailing commas are accepted, as these files are hand-edited, and one that still cannot be read is left untouched instead of being replaced. The workspace is passed as `${workspaceFolder}` so one entry serves every checkout; where that names a workspace nobody serves — Cursor resolves it against the folder holding the config, which for a home-directory file is the home directory, and some clients do not expand it at all — the bridge tries the directory it was started in and then the single window that is serving, refusing to guess when several are open. The transport is hand-written rather than taken from `@modelcontextprotocol/sdk`, which measured at 850 KB against a 182 KB bundle for two tools; it is stateless, issuing no session id, so a client cannot fail every call by losing one.

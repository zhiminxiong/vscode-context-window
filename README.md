...
- `contextView.contextWindow.jumpMode` — What a click in the Context Window jumps to. Change it from the pull-up at the right of the Context footer. Possible values:
    ...
    - `relation` — Find Relation. Click a function to list its direct callers in Context, then click one to open that location in the panel (not the main editor). **Find Relation (in ContextView)** from the editor or Context right-click menu switches to this mode and runs the lookup.
...
- `contextView.contextWindow.doubleClickSelectsBracketPair` — **In the main editor**, double-clicking next to a bracket `(` `[` `{` `<` or a quote `"` `'` `` ` `` selects the whole matching pair including the delimiters. Complements VS Code’s built-in `editor.doubleClickSelectsBlock`, which selects only the content inside. **On by default.** Toggle from the editor right-click menu or the keyboard shortcut.
    - VS Code never exposes double-click events to extensions, so the double-click has to be inferred from the shape of the selection: word selection always yields one complete run of same-class characters, while a drag grows character by character and is therefore rejected. This shape check is deliberately version-agnostic — VS Code changed its word-selection behaviour next to punctuation in late 2025, so older builds and derivatives such as Cursor select the *space* left of a bracket where newer builds select the bracket itself, and both are accepted.
    - One case cannot be filtered out in advance: a drag that happens to stop exactly on a run boundary produces the same selection as a double-click — the `}` of a `});` line is the classic example. If that happens, simply keep dragging and normal drag selection resumes from the point where you pressed down. Turn this setting off if you would rather leave the main editor alone; the Context Window has its own switch.
- `contextView.contextWindow.contextDoubleClickSelectsBracketPair` — The same feature **in the Context Window**, bound to a **right-button** double-click. On by default, and independent of the main-editor switch above — the Context Window runs on Monaco and does get a reliable click count, so it has none of the drag ambiguity described above. This is what the `{si}` indicator in the footer shows and toggles.

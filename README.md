...
- `contextView.contextWindow.doubleClickSelectsBracketPair` — **In the main editor**, double-clicking next to a bracket `(` `[` `{` `<` or a quote `"` `'` `` ` `` selects the whole matching pair including the delimiters. Complements VS Code’s built-in `editor.doubleClickSelectsBlock`, which selects only the content inside. **On by default.** Toggle from the editor right-click menu or the keyboard shortcut.
    - VS Code never exposes double-click events to extensions, so the double-click has to be inferred by replaying VS Code’s own word-selection algorithm: the pair is only widened when the incoming selection matches that result exactly. Dragging away from a bracket expands character by character and is therefore rejected.
    - One case cannot be filtered out in advance: when the click lands exactly on the start of the word-selection range, a double-click and a drag from that point to the end of the range produce an identical selection — the `}` of a `});` line is the classic example. If that happens, simply keep dragging and normal drag selection resumes from the point where you pressed down. Turn this setting off if you would rather leave the main editor alone; the Context Window has its own switch.

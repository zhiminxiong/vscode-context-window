...
- Context Window extras
  ...
  - Source Insight–style double-click to select a whole bracket/quote pair (left-click in the main editor, right-button double-click in the Context Window). One setting, also toggled from the footer `{si}` indicator; it differs from VS Code’s default double-click selection.
...
- Relation
  ...
  **Find Relation** uses the same incoming walk as **Show Relation**: callers of a function, or reference sites of a variable / field / type. Results go into the sidebar REFERENCES view (without paging). Click a result to jump the editor there.
...
- `contextView.contextWindow.jumpMode` — What a click in the Context Window jumps to. Change it from the pull-up at the right of the Context footer. Possible values:
    ...
    - `relation` — Find Relation. Same as Show Relation's incoming side: callers of a function, or references of a variable / field / type, listed in Context. Click one to open that location in the panel (not the main editor). **Find Relation (in ContextView)** from the editor or Context right-click menu switches to this mode and runs the lookup.
...
- `contextView.contextWindow.doubleClickSelectsBracketPair` — Double-clicking next to a bracket `(` `[` `{` `<` or a quote `"` `'` `` ` `` selects the whole matching pair including the delimiters. **One switch** for the main editor (left double-click) and the Context Window (right-button double-click). Complements VS Code’s built-in `editor.doubleClickSelectsBlock`, which selects only the content inside. **On by default.** Toggle from the footer `{si}` indicator, the editor right-click menu, or the keyboard shortcut.
    - In the main editor, VS Code never exposes double-click events to extensions, so the double-click has to be inferred from the shape of the selection: word selection always yields one complete run of same-class characters, while a drag grows character by character and is therefore rejected. This shape check is deliberately version-agnostic — VS Code changed its word-selection behaviour next to punctuation in late 2025, so older builds and derivatives such as Cursor select the *space* left of a bracket where newer builds select the bracket itself, and both are accepted.
    - One case cannot be filtered out in advance: a drag that happens to stop exactly on a run boundary produces the same selection as a double-click — the `}` of a `});` line is the classic example. If that happens, simply keep dragging and normal drag selection resumes from the point where you pressed down. The Context Window uses Monaco’s real click count, so it has none of that drag ambiguity.
- `contextView.contextWindow.doubleClickSelectsBracketPairDragGuard` — Extra drag guard in milliseconds for the **main editor**. Default `60`, `0` disables it. Rejects a selection arriving sooner than this after the mouse went down, i.e. the first frame of a press-and-drag.
- `contextView.contextWindow.doubleClickSelectsBracketPairConfirmDelay` — Confirmation window in milliseconds for the **main editor**. Default `90`. The pair selection is held back this long and dropped if the mouse keeps moving, so nothing touches the selection until a double-click is certain. Note that no purely time-based guard can stop a slow drag, since crossing one character may take hundreds of milliseconds — that is what the word-selection check above is for. Lower it for a snappier response, raise it if a slow drag still triggers the pair selection.
